	...
	for 韻 in ('東','冬','鍾','江','支','支A','支B','脂','脂A','脂B','之','微','魚','虞','模','齊','佳','皆','灰','咍','眞','眞A','眞B','諄','諄A','臻','文','欣','元','魂','痕','寒','桓','刪','山','先','仙','仙A','仙B','蕭','宵','宵A','宵B','肴','豪','歌','戈','麻','陽','唐','庚','耕','清','清A','青','蒸','登','尤','侯','幽','侵','侵A','侵B','覃','談','鹽','鹽A','鹽B','添','咸','銜','嚴','凡','祭','祭A','祭B','泰','夬','廢'):
		...
		for 聲 in '平上去入':
			...
			for 開合 in '開合':
				...
				for 等 in range(1, 5):
					f.write('<tr>')
					f.write(f'<td rowspan="32">{韻}</td>' if not 韻printed else '')  # 韻
					f.write(f'<td rowspan="8">{聲}</td>' if not 聲printed else '')  # 聲
					f.write(f'<td rowspan="4">{開合}</td>' if not 開合printed else '')  # 開合
